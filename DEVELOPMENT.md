...
Use the `NPM Deploy` GitHub Actions workflow for normal releases:

1. Get `main` into the exact state that should ship, including the intended
   `package.json` version.
2. In GitHub Actions, run `NPM Deploy` from `main`.
...
branch into the exact state that should ship, run `NPM Deploy` from that branch,
...
`NPM Deploy` workflow still verifies that the GitHub Release tag exactly matches
...
run `NPM Deploy`, finalize the GitHub Release, then let GitHub Actions publish to
npm.
